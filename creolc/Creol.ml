...
module Statement =
  struct
	...
	Unfortunately, this implementation is not correct.  It does not
	work correctly (or as expected), if one branch waits on two labels
	{i or} if two branches wait on the same labels, since the arrival
	of the first message makes the choice.

	This does not break partial correctness, however, since if one
	branch terminates, all requirements are satisfied. *)
    let lower_await =
      function
	  Await (a, g) ->
	    let sort_clause expr =
	      let rec sort_clause (label, term) =
		function
		    Label _ as l -> (l::label, term)
		  | Binary (b, And, left, right) ->
		      let (ll, lo) = sort_clause (label, term) right
		      in sort_clause (ll, lo) left
		  | FuncCall (b, "&&", [left ; right]) ->
		      let (ll, lo) = sort_clause (label, term) right
		      in sort_clause (ll, lo) left
		  | (Binary (_, Or, _, _) | FuncCall (_, "||", [_; _])) ->
		      assert false (* DNF! *)
		  | other -> (label, other::term)
	      in
	      let rec rebuild_clause =
		function
		    [] -> assert false
		  | [t] -> t
		  | t::l -> Binary(Expression.note t, And, t, rebuild_clause l)
	      in
		match sort_clause ([], []) expr with
		    ([], term) -> rebuild_clause term
		  | (labels, []) -> rebuild_clause labels
		  | (labels, term) ->
		...
		| FuncCall (b, "||", [left; right]) ->
		    Choice (a, split_choice left, split_choice right)
		| g -> split_sequence (sort_clause g)
	    in
