...
let rec expand_statement cls mtd future_decls fresh_name =
  let future_decl l t =
    { VarDecl.name = l; var_type = t; init = None; file = ""; line = 0 }
  in
  let rec make fn =
    let Misc.FreshName (l, fn') = fn () in
      (l, fn')
  in
  function
	...
	let (future_decls', fresh_name', t') =
          expand_statement cls mtd future_decls fresh_name t
        in
	let (future_decls'', fresh_name'', f') =
          expand_statement cls mtd future_decls' fresh_name' f
        in
	  (future_decls'', fresh_name'', If(a, c, t', f'))
    | While (a, c, i, b) ->
	let (future_decls', fresh_name', b') =
          expand_statement cls mtd future_decls fresh_name b
        in
	  (future_decls', fresh_name', While (a, c, i, b'))
    | DoWhile (a, c, i, b) ->
	expand_statement cls mtd future_decls fresh_name
          (Sequence (a, b, While (a, c, i, b)))
    | Sequence (a, s1, s2) ->
	let (future_decls', fresh_name', s1') =
          expand_statement cls mtd future_decls fresh_name s1
        in
	let (future_decls'', fresh_name'', s2') =
          expand_statement cls mtd future_decls' fresh_name' s2
        in
    ...
    match mtd.Method.body with
	  ...
	  and (vars', init) =
	    expand_method_variables (Statement.note mb)
	      (future_decls @ mtd.Method.vars)
	  in
	    { mtd with Method.vars = vars' ;
...
let pass input =
  let rec expand_with cls w =
    { w with With.methods = List.map (expand_method cls) w.With.methods }
